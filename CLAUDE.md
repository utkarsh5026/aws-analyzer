...
in `analyzers/` (`s3.py`, `dynamodb.py`; `bedrock_kb.py` is planned) that a user pastes into a notebook cell or
uploads next to a notebook and `import`s. There is no package, no `setup.py` / `pyproject.toml`, and no build step.
...
- **No imports between analyzers and no shared module.** Each file must work alone in a notebook. Helpers that
  both files need (`human_size`, `human_money`, `_require`, `_in_notebook`, `_esc`, the render blocks and
  `_render_html` / `_render_text`, `_friendly_errors`, `View._progress`, `View.help`) are deliberately
  duplicated. When you fix or change one of them, check the copy in the other analyzer.
...
- **Read-only against AWS.** Nothing writes to a bucket or table (e.g. S3 `deleted()` shows the restore call but
  never runs it). Keep it that way; README lists the read-only IAM permissions per service, so update that list
...
2. **Data models**: `@dataclass`es that the analyzer returns (`ObjectInfo`, `PrefixSummary`, `BucketConfig`,
   `ItemPage`, `TableInfo`, `TableProfile`, ...). Some have a `to_df()` that lazily requires pandas.
3. **Pure analysis**: module-level functions with **no AWS calls** (`summarize_objects`, `simulate_lifecycle_objects`,
   `explain_policy`, `profile_items`, `build_filter`, `*_findings`, `*_monthly_cost`, ...). They are public API:
   users run them on S3 Inventory rows or DynamoDB exports. Put new analysis logic here when it doesn't need AWS,
...
- Methods build a list of render blocks (`_Title`, `_Cards`, `_Table`, `_Note`, `_Text`, and in S3 also `_Frame`,
  `_Image`, `_Link`, `_Media`) and pass them to `self._show(blocks)`, which renders HTML in Jupyter or plain text
...
- `DynamoDBView` keeps `self._pager` so `more()` continues the last `scan` / `query` / `sql`.

Cost estimates come from module-level price tables (`S3_PRICES`, `DYNAMODB_PRICES`, us-east-1 list prices) that
callers override with `prices={...}`; the View shows whether list prices or the caller's prices were used.
...
- `docs/` is the guide site: plain static HTML published to GitHub Pages by `.github/workflows/pages.yml` on pushes
  ...
  guide (`s3.html`, `dynamodb.html`) that links back to it. A new analyzer gets its own `docs/<service>.html`, a card
  on `index.html` and a link in README. `index.html` also forwards old `/#section` links (from when it was the S3
  guide) to `s3.html`, so keep its own ids in the `own` list there. The screenshots
  (`docs/images/*-{light,dark}.webp`) are the tool's own output from a demo bucket and demo tables.
